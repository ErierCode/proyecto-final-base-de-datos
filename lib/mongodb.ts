...
export class MongoDBClient {
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          error:
            "Formato de consulta no reconocido. Use comandos db.*, SQL, o JSON estructurado",
        };
      }
      ...
      if (parsedQuery.collection && parsedQuery.operation) {
        ...
        return {
          success: false,
          error:
            'La consulta debe incluir "collection" y "operation" o ser un comando directo',
        };
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Error ejecutando comando directo",
      };
    }
  }
  ...
  private extractDocumentFromInsertOne(command: string): any | null {
    ...
    try {
      ...
      match = command.match(/\.insertOne\(\s*(\{[\s\S]*?\})\s*\)/);
      if (match) {
        let jsonStr = match[1];
  ...
  private isJSONCommand(query: string): boolean {
    try {
      ...
      return (
        parsed.operation && parsed.collection && typeof parsed === "object"
      );
    } catch {
      return false;
    }
  ...
  private async executeJSONCommand(
    query: string,
    db: any
  ): Promise<{
    success: boolean;
  ...
  private async executeSelectQuery(
    ...
    try {
      ...
      const selectMatch = query.match(
        /SELECT\s+\*\s+FROM\s+(\w+)(?:\s+WHERE\s+(.+?))?(?:\s+LIMIT\s+(\d+))?/i
      );
      ...
      if (!selectMatch) {
        return {
          success: false,
          error:
            "Formato SELECT no reconocido. Use: SELECT * FROM collection [WHERE condition] [LIMIT n]",
        };
      }
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Error ejecutando SELECT",
      };
    }
  }

  // Ejecutar consultas INSERT (SQL to MongoDB)
  private async executeInsertQuery(
    query: string,
    db: any
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (!insertMatch) {
        return {
          success: false,
          error:
            "Formato INSERT no reconocido. Use: INSERT INTO collection VALUES (field1, field2, ...)",
        };
      }
      ...
      return {
        success: true,
        data: [
          { insertedId: result.insertedId, acknowledged: result.acknowledged },
        ],
        columns: ["insertedId", "acknowledged"],
        rowCount: 1,
      };
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Error ejecutando INSERT",
      };
    }
  }
    ...
    try {
      ...
      if (!updateMatch) {
        return {
          success: false,
          error:
            "Formato UPDATE no reconocido. Use: UPDATE collection SET field=value [WHERE condition]",
        };
      }
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Error ejecutando UPDATE",
      };
    }
  }
    ...
    try {
      ...
      if (!deleteMatch) {
        return {
          success: false,
          error:
            "Formato DELETE no reconocido. Use: DELETE FROM collection [WHERE condition]",
        };
      }
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Error ejecutando DELETE",
      };
    }
  }
  ...
  private async executeCreateTableQuery(
    query: string,
    db: any
  ): Promise<{
    success: boolean;
    ...
    try {
      ...
      if (!createMatch) {
        return {
          success: false,
          error:
            "Formato CREATE TABLE no reconocido. Use: CREATE TABLE collection_name",
        };
      }
